...
export const DrawerSetupCreateTableCoulumns =
  `id TEXT  PRIMARY KEY NOT NULL, InitialBillNumber TEXT, Terminal TEXT, StartInvoiceNumber TEXT, EndInvoiceNumber TEXT, StartDate  TEXT, initialCash  TEXT, DepositCash  TEXT, CashRefund TEXT, CashSales TEXT,` +
...
export const DrawerSetupInsertCoulumns = `id, InitialBillNumber, Terminal, StartInvoiceNumber, EndInvoiceNumber, StartDate, initialCash, DepositCash, CashRefund, CashSales, withDraw, estimatedAmountinDrawer, creditSales, creditRefunds, cardSale, cardReturn, isInitialCashSet, isInitialLogin`;
...
export const InsertDrawerSetup = async values => {
  ...
  for (let i = 0; i < values?.length; ++i) {
    InsertDataQuery =
      ...
      values[i].id +
      "','" +
      values[i].InitialBillNumber +
      "','" +
      values[i].Terminal +
      "','" +
      values[i].StartInvoiceNumber +
      "','" +
      values[i].EndInvoiceNumber +
      "','" +
      values[i].StartDate +
      "','" +
